...
export function SchoolFeed({ userType, simple }: SchoolFeedProps) {
  ...
  if (simple) {
    return (
      <div className="space-y-3 w-full px-0 sm:px-2 md:px-4 lg:px-8">
        <div className="flex gap-2 w-full">
          <Select value={filterGrade} onValueChange={setFilterGrade}>
            <SelectTrigger className="w-full h-9 border rounded font-bold">
        ...
        <div className="space-y-3 w-full">
          {filteredPosts.map((post) => (
            <Card key={post.id} className="border rounded p-3 w-full">
              ...
              <CardContent>
                ...
                <div className="flex gap-2 text-xs">
                  ...
                  <Badge>{getTargetAudienceLabel(post.targetAudience)}</Badge>
                </div>
              </CardContent>
            </Card>
          ))}
          {filteredPosts.length === 0 && (
            <Card className="border rounded text-center p-6 w-full">
              <CardContent>
                <p className="text-base text-muted-foreground">No posts match your current filters.</p>
              </CardContent>
            </Card>
          )}
        </div>
  ...
  // ...existing code...
  return (
    <div className="space-y-8 w-full px-0 sm:px-2 md:px-4 lg:px-8">
      <div className="flex gap-4 mb-6 w-full">
        <Select value={filterGrade} onValueChange={setFilterGrade}>
          <SelectTrigger className="w-48 h-10 border rounded font-bold">
            <SelectValue placeholder="All Grades" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All Grades</SelectItem>
      ...
      <div className="space-y-8 w-full">
        {filteredPosts.map((post) => (
          <div className="w-full"><FeedPostCard key={post.id} post={post} /></div>
        ))}
        {filteredPosts.length === 0 && (
          <Card className="border rounded text-center p-6 w-full">
            <CardContent>
